...
if ($_POST) {
    ...
    $cari = mysqli_query($koneksi,"SELECT opsi FROM mapel WHERE id_mapel = $id_mapel");
    ...
    if ($zip->open($new_name_path) === true) {
        ...
        foreach ($relation_image as $key => $value) {
            ...
            mysqli_query($koneksi, "INSERT INTO file_pendukung (id_mapel, nama_file) VALUES ('$id_mapel', '$imagenew_name')");
    ...
    if (count($quesions) < $jml_soal) {
        ...
        exit;
    } else if (count($quesions) > $jml_soal) {
        mysqli_query($koneksi, "UPDATE mapel SET jml_soal = ".count($quesions)." WHERE id_mapel = $id_mapel");
    }
